...
const AdminPanel: React.FC<AdminPanelProps> = ({ gameState, onUpdate }) => {
  ...
  if (gameState && gameState.status === 'ENDED') {
    return (
      <div className="bg-white p-8 rounded-3xl shadow-xl border-2 border-indigo-50 text-center animate-in fade-in zoom-in duration-300">
        <h2 className="text-3xl font-black text-gray-800 mb-4">🏁 Ván chơi đã kết thúc</h2>
        <p className="text-gray-500 mb-8">Cảm ơn mọi người đã tham gia!</p>
        <button
          onClick={() => onUpdate(null)}
          className="bg-indigo-600 text-white py-3 px-8 rounded-2xl font-black text-lg hover:bg-indigo-700 transition-all shadow-lg active:scale-95"
        >
          🔄 Tạo ván mới
        </button>
      </div>
